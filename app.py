...
#Function to load the audio file,resnet_blocks=2,attn_blocks=4, num_attn_heads=4,base_channels=32,dropout=0,kernel_size=5,distribute_zero_label=False
...
#Function wich classifies the code 

#function for classifier
def classify_audio_clip(clip):
# Returns whether or not the classifier thinks the given clip came from AI generation.
# :param clip: torch tensor containing audio waveform data (get it from load_audio) :return: The probability of the audio clip being AI-generated.
# மாய
...
def main():
    ...
    if uploaded_file is not None:
        if st.button("Analyze Audio"):
           ...
           with col3:
                st.info("Disclaimer")
                st.warning("Input files must be taken care of the format")
